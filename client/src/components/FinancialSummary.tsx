    ...
    <div className="border-t border-gray-200 p-4 bg-gray-50">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-white rounded-lg p-3 shadow-sm">
          ...
          <div className="font-mono font-medium text-red-500">-${financialData.thisWeekExpenses.toFixed(2)}</div>
        </div>
        <div className="bg-white rounded-lg p-3 shadow-sm">
          <div className="text-sm text-gray-500">Next Week</div>
          <div className="font-mono font-medium text-red-500">-${financialData.nextWeekExpenses.toFixed(2)}</div>
        </div>
        <div className="bg-white rounded-lg p-3 shadow-sm">
          <div className="text-sm text-gray-500">This Month</div>
          <div className="font-mono font-medium text-red-500">-${financialData.thisMonthExpenses.toFixed(2)}</div>
        </div>
        <div className="bg-white rounded-lg p-3 shadow-sm">
          <div className="text-sm text-gray-500">Income</div>
          <div className="font-mono font-medium text-green-500">+${financialData.totalIncome.toFixed(2)}</div>
        </div>
      </div>
      <div className="mt-3 bg-white rounded-lg p-3 shadow-sm">
        <div className="flex justify-between">
          ...
          <span className={`font-mono font-medium ${financialData.balance >= 0 ? 'text-blue-500' : 'text-red-500'}`}>
            {financialData.balance >= 0 ? '+' : '-'}${Math.abs(financialData.balance).toFixed(2)}
          </span>
        </div>
        ...
        <div className="mt-1 flex justify-between text-xs text-gray-500">
          <span>Spent: ${financialData.thisMonthExpenses.toFixed(2)}</span>
          <span>Savings: ${Math.max(0, financialData.balance).toFixed(2)}</span>
        </div>
      </div>
    </div>
  );
